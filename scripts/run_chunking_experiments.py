#!/usr/bin/env python3
"""Run Phase 8 chunking experiments on the development set only."""
...
RESULT_DIR = PROJECT_ROOT / "results" / "chunking"
BASELINE_TOP_K = 3
EVALUATION_DEPTH = 10

EXPERIMENTS = [
    {"experiment_id": "chunk_256_0", "chunk_size": 256, "chunk_overlap": 0},
    {"experiment_id": "chunk_256_64", "chunk_size": 256, "chunk_overlap": 64},
    {"experiment_id": "chunk_512_0", "chunk_size": 512, "chunk_overlap": 0},
    {"experiment_id": "chunk_512_64", "chunk_size": 512, "chunk_overlap": 64},
    {"experiment_id": "chunk_512_128", "chunk_size": 512, "chunk_overlap": 128},
    {"experiment_id": "chunk_1024_128", "chunk_size": 1024, "chunk_overlap": 128},
]
...
def evaluate_experiment(
    ...
    collection_name = f"{settings.chroma_collection}_{experiment['experiment_id']}"
...
def main() -> int:
    ...
    for experiment in EXPERIMENTS:
        print(
            ...
            f"(chunk_size={experiment['chunk_size']}, chunk_overlap={experiment['chunk_overlap']})"
        )
        per_question, summary, question_type_rows, provider_rows = evaluate_experiment(
            experiment, rows, settings, embedder, provider_by_doc_id
        )
        all_per_question.extend(per_question)
        all_out_of_scope.extend(
            row for row in per_question if row["ground_truth_type"] == "out_of_scope"
        )
    ...
    write_csv(RESULT_DIR / "chunking_per_question.csv", all_per_question, PER_QUESTION_FIELDS)
    write_csv(RESULT_DIR / "chunking_out_of_scope.csv", all_out_of_scope, PER_QUESTION_FIELDS)
    summary_fields = [
        "experiment_id",
        "chunk_size",
        "chunk_overlap",
    ...
    write_csv(RESULT_DIR / "chunking_summary.csv", all_summaries, summary_fields)
    write_csv(
        RESULT_DIR / "chunking_by_question_type.csv",
        all_question_type_rows,
        ["experiment_id", "chunk_size", "chunk_overlap", "question_type", "count", "hit_rate_at_1", "hit_rate_at_3", "mrr"],
    )
    write_csv(
        RESULT_DIR / "chunking_by_provider.csv",
        all_provider_rows,
        ["experiment_id", "chunk_size", "chunk_overlap", "provider", "count", "hit_rate_at_1", "hit_rate_at_3", "mrr"],
    )
    ...
    write_csv(RESULT_DIR / "chunking_tracked_questions.csv", tracked, PER_QUESTION_FIELDS)
    (RESULT_DIR / "chunking_summary.json").write_text(
